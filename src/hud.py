...
class Hud:
    ...
    def draw(self):
        ...
        hp_offset = 32
        if p.hp > 3: hp_offset = 40
        if p.hp > 6: hp_offset = 48
        pyxel.blt(x, y, 0, hp_offset, 48, 8, 8) # heart
        pyxel.text(x+2, y+8, str(p.hp) + "/" + str(p.max_hp), 7) # hp text
        ...
        pyxel.text(128, 10, str(self.world.daylight_control.sec_cnt), 2)

        pyxel.text(128, 20, str(self.world.current_map.player.tile_x*8) + "," + str(self.world.current_map.player.tile_y*8), 7)

        pyxel.text(128, 30, str(pyxel.mouse_x - 40 + self.world.current_map.cam.x) + "," + str(pyxel.mouse_y - 8 + self.world.current_map.cam.y), 7)
